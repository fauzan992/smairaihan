...
export const BarcodeScannerModal: React.FC<BarcodeScannerModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/75 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4">
      <div className="bg-white rounded-2xl max-w-lg w-full p-5 sm:p-6 shadow-2xl border border-slate-100 flex flex-col max-h-[92vh] overflow-y-auto">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="font-bold text-slate-800 text-lg leading-tight">Scanner Barcode & QR NISN</h3>
                <span className="bg-emerald-100 text-emerald-800 text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1">
                  <ShieldCheck className="w-3 h-3 text-emerald-600" /> Anti-Scan Ganda
                </span>
              </div>
        ...
        <div className="grid grid-cols-3 gap-1 bg-slate-100 p-1 rounded-xl mb-3">
          ...
          >
            <Camera className="w-3.5 h-3.5" /> Kamera Belakang HP
          </button>
          <button
        ...
        <div className="mb-3">
          <div className="flex justify-between items-center mb-1.5">
            ...
            <span className="text-[11px] font-bold text-slate-500 bg-slate-100 px-2 py-0.5 rounded-md">
              Total Terrecord Hari Ini: <strong className="text-emerald-700">{totalScannedCount}</strong>
            </span>
          </div>
        ...
        <div className="relative mb-3 min-h-[260px] bg-slate-950 rounded-2xl overflow-hidden flex flex-col items-center justify-center p-2 border border-slate-800 shadow-inner">
          {scannerMode === 'camera' && (
            <div className="w-full relative flex flex-col items-center">
              <div id="reader-camera" className="w-full max-w-[320px] rounded-xl overflow-hidden text-white text-xs"></div>
              
              <div className="mt-2 text-center">
                <span className="text-[11px] font-medium text-emerald-400 bg-emerald-950/80 px-3 py-1 rounded-full border border-emerald-800/60 inline-flex items-center gap-1.5">
        ...
        {scanResult && (
          <div className="p-3.5 bg-emerald-50 border-2 border-emerald-300 rounded-xl mb-3 animate-in fade-in slide-in-from-bottom-2 duration-200 shadow-sm">
            <div className="flex items-start gap-3">
              <CheckCircle2 className="w-6 h-6 text-emerald-600 shrink-0 mt-0.5" />
              <div className="flex-1">
                <div className="flex justify-between items-center">
                  <span className="text-[11px] font-extrabold text-emerald-800 uppercase tracking-wider bg-emerald-200/80 px-2 py-0.5 rounded-md">
                ...
                <h4 className="font-bold text-slate-900 text-base mt-1">{scanResult.studentName}</h4>
                <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-700 mt-1 font-medium">
                  <span>NISN: <strong className="font-mono text-emerald-800">{scanResult.nisn}</strong></span>
                  <span>Kelas: <strong>{scanResult.className}</strong></span>
                  <span>Status: <strong className="text-emerald-700 font-bold">{scanResult.status}</strong></span>
                </div>
              </div>
            </div>
          </div>
        ...
        {studentsList.length > 0 && (
          <div className="mb-3">
            ...
            <div className="flex flex-wrap gap-1 max-h-24 overflow-y-auto p-1.5 bg-slate-50 rounded-lg border border-slate-100">
              {studentsList.slice(0, 10).map((st) => {
                ...
                return (
                  ...
                  >
                    <span className={`font-mono font-bold ${isAlreadyScanned ? 'text-slate-400' : 'text-emerald-600'}`}>{st.nisn}</span> - {st.name} ({st.className})
        ...
        <div className="flex justify-between items-center pt-3 border-t border-slate-100">
          <p className="text-[11px] text-slate-500">
            Kamera otomatis tetap terbuka. Klik Selesai jika presensi telah usai.
          </p>
          <button
            onClick={onClose}
            className="px-5 py-2 bg-emerald-700 hover:bg-emerald-800 text-white rounded-xl text-xs font-bold transition-colors cursor-pointer shadow-sm"
          >
            Selesai
          </button>
        </div>
